...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (rawFormData) {
    ...
    if (name && passwordFieldValid) {
      ...
      const changePasswordResult = await changePassword.safeParseAsync({
        currentPassword,
        newPassword,
        confirmPassword,
      });
      if (changePasswordResult.error) {
        ...
        const currentPasswordError =
          flattened.fieldErrors["currentPassword"]?.[0] ?? "";
        const newPasswordError =
          flattened.fieldErrors["newPassword"]?.[0] ?? "";
...
export default function Profile({ actionData }: Route.ComponentProps) {
  ...
  const successEmailModalToggle = useToggle();
  const failedEmailModalToggle = useToggle();
  ...
  const [fieldError, setFieldError] =
    useState<Partial<FieldErrorTypes>>(defaultFieldErrors);
  ...
  useEffect(() => {
    ...
    if (!fetcher.data.success) {
      return failedEmailModalToggle.open();
    }

    successEmailModalToggle.open();
  }, [fetcher.data]);

  const sendEmailVerification = () => {
    fetcher.submit({ verifyEmail: true }, { method: "post" });
  };

  if (!user) {
    return <Spinner spinSize="w-12 h-12" />;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-800 bg-opacity-90 flex flex-col items-center py-12 sm:px-4">
      ...
      {successEmailModalToggle.isOpen && (
        <EmailVerificationModal
          details={fetcher.data.message}
          onClose={successEmailModalToggle.close}
        />
      )}

      {failedEmailModalToggle.isOpen && (
        <EmailVerificationModal
          message="Something went wrong"
          details={fetcher.data.message}
          onClose={failedEmailModalToggle.close}
        />
      )}
