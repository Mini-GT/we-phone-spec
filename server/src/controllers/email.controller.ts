...
const updateUserEmailVerification = async (req: Request, res: Response) => {
  ...
  if (!result) {
    return res
      .status(401)
      .json({ success: false, message: "Couldn't send email verification" });
  }

  res.status(200).json({
    success: true,
    message: "Please check your email inbox/spambox",
  });
};
...
const resetPassword = async (req: Request, res: Response) => {
  ...
  if (!user) {
    return res
      .status(400)
      .json("Couldn't reset password please try again later");
  }
